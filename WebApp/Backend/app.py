...
def insert_university():
    ...
    try:
        with conn.cursor(dictionary=True) as cursor:
            ...
            results = cursor.fetchall()  # Fetch all to clear the buffer

            if results:
                university_info = results[0]  # Pick the first row
            else:
    ...
    try:
        with conn.cursor(dictionary=True) as cursor:
            ...
            if work_info:
                ...
                print("Modified WorkInfo: ", work_info)
...
def get_music():
    ...
    file_url = "/static/audio/{}".format(filename)  # Construct the file URL
...
def delete_music():
    ...
    try:
        ...
        return jsonify({"message": "Music with ID {} deleted successfully.".format(music_id)}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
